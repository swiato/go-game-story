...
Canvas.prototype.removeListener = function(event, callback) {
  let callbackArray = this.listeners[event];
...
Stones.prototype.drawStone = function(ctx, type, ox, oy, scale) {
  ...
  if(!stone) { // BW
    ctx.fillStyle = (type == C.WHITE || type == C.DIM_WHITE) ? '#FFFFFF' : '#000000';
    ctx.beginPath();
    ctx.arc(ox, oy, this.stoneR*scale, 2*Math.PI, false);
    ctx.fill();

    if(type == C.WHITE || type == C.DIM_WHITE) {
